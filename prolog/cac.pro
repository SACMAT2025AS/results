...
initAdmC :-
    ansi_format([fg(green)], "[CryptoAC] initAdmC~n", []),
...
initUserC(USER) :-
    ansi_format([fg(green)], "[CryptoAC] initUserC ~a ~n", [USER]),
...
addUserC(USER) :-
    ansi_format([fg(green)], "[CryptoAC] addUserC ~a ~n", [USER]),
...
deleteUserC(USER) :-
    ansi_format([fg(green)], "[CryptoAC] deleteUserC ~a ~n", [USER]),
...
addRoleC(ROLE) :-
    ansi_format([fg(green)], "[CryptoAC] addRoleC ~a ~n", [ROLE]),
...
deleteRoleC(ROLE) :-
    ansi_format([fg(green)], "[CryptoAC] deleteRoleC ~a ~n", [ROLE]),
...
addResourceC(FILE) :-
    ansi_format([fg(green)], "[CryptoAC] addResourceC ~a ~n", [FILE]),
...
deleteResourceC(FILE) :-
    ansi_format([fg(green)], "[CryptoAC] deleteResourceC ~a ~n", [FILE]),
...
assignUserToRoleC(USER, ROLE) :-
    ...
    ansi_format([fg(green)], "[CryptoAC] assignUserToRoleC ~a ~a ~n", [USER, ROLE]),
...
revokeUserFromRoleC(USER, ROLE) :-
    ...
    ansi_format([fg(green)], "[CryptoAC] revokeUserFromRoleC ~a ~a ~n", [USER, ROLE]),
...
assignPermissionToRoleC(ROLE, [OP|OPs], FILE) :-
    ...
    ansi_format([fg(green)], "[CryptoAC] assignPermissionToRoleC ~a ~a ~a ~n", [ROLE, OP, FILE]),
...
revokePermissionFromRoleC(ROLE, [OP|OPs], FILE) :-
    ...
    ansi_format([fg(green)], "[CryptoAC] revokePermissionFromRoleC ~a ~a ~a ~n", [ROLE, OP, FILE]),
...
prepareReencryptionC(FILE) :-
    ansi_format([fg(green)], "[CryptoAC] prepareReencryptionC ~a ~n", [FILE]),
...
reencryptResourceC(FILE) :-
    ansi_format([fg(green)], "[CryptoAC] reencryptResourceC ~a ~n", [FILE]),
...
rotateRoleKeyUserRoleC(ROLE) :-
    ansi_format([fg(green)], "[CryptoAC] rotateRoleKeyUserRoleC ~a ~n", [ROLE]),
...
rotateRoleKeyPermissionsC(ROLE) :-
    ansi_format([fg(green)], "[CryptoAC] rotateRoleKeyPermissionsC ~a ~n", [ROLE]),
...
readResourceC(USER, FILE) :-
    ansi_format([fg(green)], "[CryptoAC] readResourceC ~a ~a ~n", [USER, FILE]),
...
writeResourceC(USER, FILE) :-
    ...
    ansi_format([fg(green)], "[CryptoAC] writeResourceC ~a ~a ~n", [USER, FILE]),
...
canUserDoC(USER, OP, FILE) :-
    userRoleC(USER, ROLE, _, ope),
    permC(ROLE, FILE, _, _, OP, ope).
...
% canUserDoViaRoleCacheLastC(USER, ROLE, OP, FILE) :-
%     (userRoleC(USER, ROLE, ROLE_V, ope); userRoleC(USER, ROLE, ROLE_V, cache)),
%     (permC(ROLE, FILE, ROLE_V, _, OP, ope); permC(ROLE, FILE, ROLE_V, _, OP, cache)).
...
canRoleDoCacheC(ROLE, OP, FILE) :-
    (permC(ROLE, FILE, _, FILE_V, OP, ope); permC(ROLE, FILE, _, FILE_V, OP, cache)),
    fileC(FILE, FILE_V, _, ope, _).
...
% canRoleDoCacheLastC(ROLE, OP, FILE) :-
%     (permC(ROLE, FILE, _, _, OP, ope); permC(ROLE, FILE, _, _, OP, cache)).
...
canUserBeCacheC(USER, ROLE) :-
    roleC(ROLE, ROLE_V, ope), % the role must be operative
    (userRoleC(USER, ROLE, ROLE_V, ope); userRoleC(USER, ROLE, ROLE_V, cache)).

% canUserBeCacheC(USER, ROLE) :-
%     (userRoleC(USER, ROLE, _, ope); userRoleC(USER, ROLE, _, cache)).

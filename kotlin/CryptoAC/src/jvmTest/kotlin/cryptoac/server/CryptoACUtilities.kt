...
import cryptoac.Parameters.cryptoACBaseAPI
...
class CryptoACUtilities {
    ...
    companion object {
                ...
                TestUtilities.getKtorClientJetty().use {
                    ...
                    val response = it.submitForm(
                        formParameters = Parameters.build {
                            ...
                            enforcement?.let { it1 -> append(ENFORCEMENT, it1) }
                        }
                    ) {
                        url("$HTTPS$cryptoACBaseAPI${API.CRYPTOAC}resources/$core/")
                    }
        ...
        fun getAssignments(
            ...
            var userRole: HashSet<UserRole>? = null
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginCryptoAC(it, loggedUser)
                    val response = it.get {
                    ...
                    if (expectedCode == OutcomeCode.CODE_000_SUCCESS) {
                        userRole = myJson.decodeFromString<HashSet<UserRole>>(response.bodyAsText())
                    } else {
                        myJson.decodeFromString<OutcomeCode>(response.bodyAsText()).apply {
                            assertEquals(expectedCode, this)
                        }
        ...
        fun getPermissions(
            ...
            var rolePermission: HashSet<RolePermission>? = null
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginCryptoAC(it, loggedUser)
                    val response = it.get {
                    ...
                    if (expectedCode == OutcomeCode.CODE_000_SUCCESS) {
                        rolePermission = myJson.decodeFromString<HashSet<RolePermission>>(response.bodyAsText())
                    } else {
                        myJson.decodeFromString<OutcomeCode>(response.bodyAsText()).apply {
                            assertEquals(expectedCode, this)
                        }

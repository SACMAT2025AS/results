...
import cryptoac.Parameters.adminCoreCACRBACMQTTParameters
...
import cryptoac.server.CryptoACUtilities.Companion.initAdmin
import io.ktor.http.*
...
internal class CryptoACRoutesTest {
    ...
    @BeforeEach
    fun setUp() {
        initAdmin(adminCoreCACRBACMQTTParameters)
    }

    @AfterEach
    fun tearDown() {
        TestUtilities.resetACServiceRBACDynSEC(null)
//        TestUtilities.resetDMServiceRBACMQTT()
    ...
    fun `get profile of admin as admin, user as user or user as admin works`() {
        ...
        run {
            ...
            assert(adminCoreCACRBACMQTTParameters.user.isAdmin == getAdminParameters!!.user.isAdmin)
            assert(adminCoreCACRBACMQTTParameters.coreType == getAdminParameters.coreType)
            assert(adminCoreCACRBACMQTTParameters.user.name == getAdminParameters.user.name)
        }
    ...
    fun `update profile of admin as admin, user as user or user as admin works`() {
        ...
        run {
            CryptoACUtilities.updateProfile(adminCoreCACRBACMQTTParameters)
        }
    ...
    fun `add user not logged-in, not logged as admin, with no username or with wrong profile fails`() {
        ...
        run {
            ...
            CryptoACUtilities.addUser(
                username = "alice",
                core = CoreType.RBAC_FILES.toString(),
    ...
    fun `delete resource works`() {
        CryptoACUtilities.addResourceForm(
            resourceName = "test",
            enforcement = Enforcement.COMBINED.toString(),
        )
    ...
    fun `assign read or read-write permissions to role over resource works`() {
        ...
        run {
            CryptoACUtilities.assignPermissionToRole(
                roleName = "employee",
                resourceName = "test",
                permission = Operation.READ.toString(),
            )
        }
        ...
        run {
            CryptoACUtilities.assignPermissionToRole(
                roleName = "employee",
                resourceName = "test",
                permission = Operation.READWRITE.toString(),
            )
        }
    ...
    fun `assign read or write or both permissions to role over resource not logged-in, not logged as admin, with no role name or resource name or null or wrong permission or with wrong profile fails`() {
        ...
        run {
            ...
            CryptoACUtilities.assignPermissionToRole(
                roleName = "employee",
                resourceName = "test",
                permission = Operation.WRITE.toString(),
                expectedCode = OutcomeCode.CODE_038_UNAUTHORIZED,
                expectedStatus = HttpStatusCode.Unauthorized,
                login = false,
            )
    ...
    fun `add combined or traditional resource form or binary works`() {
        ...
        run {
            ...
            CryptoACUtilities.addResourceForm(
                resourceName = "test2",
                enforcement = Enforcement.TRADITIONAL.toString(),
            )
        }
    ...
    @Test
    fun `add combined or traditional resource form or binary not logged-in, with no resource name or content, null or wrong enforcement or or with wrong profile fails`() {
        ...
        /** add resource form with null or wrong enforcement */
        run {
            CryptoACUtilities.addResourceForm(
                resourceName = "test",
                enforcement = null,
        ...
        /** add resource binary with null or wrong enforcement */
        run {
            CryptoACUtilities.addResourceBinary(
                resourceName = "test",
                resourceContent = "content".inputStream(),

package cryptoac.view.content.newProfile
...
import cryptoac.view.Themes.plainPaperTitleStyle
import cryptoac.view.components.custom.*
import cryptoac.view.components.materialui.grid
...
data class NewProfileState(
    ...
    var coreTypeState: CoreType = CoreType.RBAC_FILES,
     ...
     */
    var availableRMImplementationsState: List<String> = getRMImplementations(CoreType.RBAC_FILES),

    /**
     * The options for the MM implementation
     * based on the selected core type
     */
    var availableMMImplementationsState: List<String> = getMMImplementations(CoreType.RBAC_FILES),

    /**
     * The options for the DM implementation
     * based on the selected core type
     */
    var availableDMImplementationsState: List<String> = getDMImplementations(CoreType.RBAC_FILES),

    /**
     * The options for the AC implementation
     * based on the selected core type
     */
    var availableACImplementationsState: List<String> = getACImplementations(CoreType.RBAC_FILES),

    /** Whether to render the value on change by the user */
...
private fun getDMImplementations(coreType: CoreType) = when (coreType) {
    CoreType.RBAC_FILES -> listOf(DMType.CRYPTOAC.toString())
    CoreType.RBAC_MQTT -> listOf(DMType.MQTT.toString())
    CoreType.ABAC_FILES -> listOf(DMType.CRYPTOAC.toString())
...
private fun getRMImplementations(coreType: CoreType) = when (coreType) {
    CoreType.RBAC_FILES -> listOf(RMType.RBAC_CRYPTOAC.toString())
    CoreType.RBAC_MQTT -> listOf(RMType.NONE.toString())
    CoreType.ABAC_FILES -> listOf(RMType.RBAC_CRYPTOAC.toString()) // TODO even though RMType.CryptoAC is not ready yet
...
private fun getACImplementations(coreType: CoreType) = when (coreType) {
    ...
    CoreType.ABAC_FILES -> listOf( // TODO even though RMType.RBAC_OPA is not ready yet
        ACType.RBAC_OPA.toString(),
        ACType.NONE.toString()
    )
    CoreType.ABAC_MQTT -> listOf(

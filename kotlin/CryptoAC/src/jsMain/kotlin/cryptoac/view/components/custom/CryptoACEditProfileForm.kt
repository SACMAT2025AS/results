package cryptoac.view.components.custom
...
import cryptoac.view.components.icons.faChevronDown
import cryptoac.view.components.icons.faChevronUp
import cryptoac.view.components.icons.faCloudUploadAlt
...
fun submitEditProfileForm(
    ...
    try {
        ...
        val mmParameters = when (props.mmTypeProp) {
            MMType.RBAC_MYSQL -> MMServiceMySQLParameters.fromMap(
                parameters = values,
                mmType = MMType.RBAC_MYSQL
            )
            MMType.ABAC_MYSQL -> MMServiceMySQLParameters.fromMap(
                parameters = values,
            ...
            MMType.ABAC_REDIS -> MMServiceRedisParameters.fromMap(
                parameters = values,
                mmType = MMType.ABAC_REDIS
            )
        }
...
fun getFieldsFromParameters(
    ...
    fields.addAll(
        when (mmType) {
            MMType.RBAC_MYSQL -> MMServiceMySQLParameters.toMap(
                mmParameters?.let { it as MMServiceMySQLParameters }
            )
            MMType.ABAC_MYSQL -> MMServiceMySQLParameters.toMap(
                mmParameters?.let { it as MMServiceMySQLParameters }
            )
            MMType.RBAC_REDIS -> MMServiceRedisParameters.toMap(
                mmParameters?.let { it as MMServiceRedisParameters }
            )
            MMType.ABAC_REDIS -> MMServiceRedisParameters.toMap(
                mmParameters?.let { it as MMServiceRedisParameters }
            )
        }
    )

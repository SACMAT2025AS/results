 ...
 * may concurrently modify the policy state, leading to
 * inconsistencies. The same reasoning applies to internal
...
class SessionController {
    ...
    companion object {
        ...
        fun addSocketToSession(session: CurrentSession, wss: DefaultWebSocketSession, core: CoreType) {
            when (core) {
                CoreType.RBAC_FILES -> {
                    val message = "Web socket is not expected for ${CoreType.RBAC_FILES}"
...
//                        (cores[userSession.username]!![core]!! as CoreCACRBACMQTT).wss = wss
                    } else {
                ...
                CoreType.ABAC_FILES -> {
                    val message = "Web socket is not expected for ${CoreType.ABAC_FILES}"
                    logger.error { message }
                    throw IllegalStateException(message)
                }
...
//                    val userSession = session.get<UserSession>()
//                    if (userSession != null && cores[userSession.username]!![core] != null) {
//                        (cores[userSession.username]!![core]!! as CoreCACABACMQTT).wss = wss
//                    } else {
//                        val message = "User is logged-in but no session was found"
//                        logger.error { message }
//                        throw IllegalStateException(message)
...
//                }
        ...
        fun getOrCreateCore(
            ...
            when (policyModel) {
                PolicyModel.RBAC -> {
                    if (!core::class.isSubclassOf(CoreCACRBAC::class)) {
...
//                {
//                    if (!core::class.isSubclassOf(CoreCACABAC::class)) {
...
/** Wrapper for [username]'s session data in the server */

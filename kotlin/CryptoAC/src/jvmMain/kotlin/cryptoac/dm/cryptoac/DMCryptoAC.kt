...
// TODO if you modify the enforcement to allow cryptographic enforcement only,
...
//  does not know what enforcement was chosen for a resource)
...
/**
 * CryptoAC test implementation of a DM.
    ...
    route(DM) {
        ...
        route(RESOURCES) {
            ...
            post {
                ...
                var newResourceVersionNumber: Int? = null
                var stream: InputStream? = null

                call.receiveMultipart().forEachPart { part ->
                    when (part) {
                        is PartData.FormItem -> {
                            if (part.name == SERVER.RESOURCE_VERSION_NUMBER) {
                                newResourceVersionNumber = part.value.toInt()
                            } else {
                ...
                if (newResourceVersionNumber == null) {
                    return@post ResponseRoutes.unprocessableEntity(
                        call,
                        "Missing ${SERVER.RESOURCE_VERSION_NUMBER} parameter",
                        CODE_019_MISSING_PARAMETERS
                    )
                }
                ...
                logger.info {
                    "User is asking to upload resource $newResourceName " +
                            "with version number $newResourceVersionNumber"
                } // TODO add username in log
                ...
                val returnCode = resourcesBeingModified[newResourceName]!!.mutex.withLock {
                    ...
                    try {
                        FileSystemManager.saveFile(
                            path = "${UPLOAD_DIRECTORY.absolutePath}/${
                                FileSystemManager.getFileKey(
                                    ...
                                    fileVersionNumber = newResourceVersionNumber!!,
                                    core = core
                                )
                            }",
                            content = stream!!,
                        ...
                        logger.warn {
                            ...
                            "and version number $newResourceVersionNumber " +
            ...
            get("{${SERVER.RESOURCE_NAME}}/{${SERVER.RESOURCE_VERSION_NUMBER}}") {
                val coreParam = call.parameters[SERVER.CORE]
                ...
                val resourceVersionNumber = call.parameters[SERVER.RESOURCE_VERSION_NUMBER]?.toInt()
                    ?: return@get ResponseRoutes.unprocessableEntity(
                        call,
                        "Missing ${SERVER.RESOURCE_VERSION_NUMBER} parameter",
                        CODE_019_MISSING_PARAMETERS
                ...
                logger.info {
                    "The user $username is asking to download resource " +
                            "$resourceName with version number $resourceVersionNumber"
                }

                /** Get the resource */
                val resource = File(
                "${DOWNLOAD_DIRECTORY.absolutePath}/" +
                    FileSystemManager.getFileKey(resourceName, resourceVersionNumber, core)
                )
                ...
                val returnCode = resourcesBeingModified[resourceName]!!.mutex.withLock {
                    ...
                    if (!resource.exists()) {
                        logger.warn { "Resource $resourceName with version number $resourceVersionNumber not found" }
             ...
             */
            put("{${SERVER.RESOURCE_NAME}}/{${SERVER.RESOURCE_VERSION_NUMBER}}") {
                val coreParam = call.parameters[SERVER.CORE]
                ...
                logger.info {
                    "The user $username is asking to move resource " +
                            "$resourceName with version number $resourceVersionNumber"
                }
            ...
            /** Delete a resource from the DM */
            delete("{${SERVER.RESOURCE_NAME}}/{${SERVER.RESOURCE_VERSION_NUMBER}}") {
                val coreParam = call.parameters[SERVER.CORE]
                ...
                logger.info {
                    "The admin is asking to delete resource $resourceName " +
                            "with version number $resourceVersionNumber"
                }

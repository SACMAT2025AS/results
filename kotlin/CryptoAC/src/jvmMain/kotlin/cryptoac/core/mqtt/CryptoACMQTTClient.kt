...
class CryptoACMQTTClient(
    ...
    /**
     * Ensure the client is connected, then invoke the super
     * function to publish the [mqttMessageCryptoAC] (containing
     * data pertaining a resource) in the [topic] with the given
     * [qos]. Finally, return a proper outcome code
     */
    fun publishDMMessage(
        topic: String,
        mqttMessageCryptoAC: MqttMessageCryptoAC,
        qos: Int,
    ): OutcomeCode {
        return if (connectSync()) {
            val message = MqttMessage(myJson.encodeToString(
                mqttMessageCryptoAC
            ).toByteArray())
            message.qos = qos
            message.isRetained = when (mqttMessageCryptoAC.type) {
                MqttMessageCryptoACType.RESOURCE_UPDATE -> true
                MqttMessageCryptoACType.RESOURCE_CONTENT -> false
                MqttMessageCryptoACType.RESOURCE_DELETE -> false
            }
    ...
    /**
     * Ensure the client is connected, then invoke the super
     * function to publish the [message] (containing data
     * pertaining the AC policy) in the [topic] with the given
     * [qos]. Finally, return a proper outcome code
     */
    fun publishACMessage(
        topic: String,
        message: MqttMessage,
    ): OutcomeCode {
        return if (connectSync()) {

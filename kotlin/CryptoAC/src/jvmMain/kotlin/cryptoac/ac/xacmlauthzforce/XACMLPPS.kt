...
import cryptoac.ac.xacmlauthzforce.XACMLPolicySet.Companion.getNextVersionNumber
...
data class XACMLPPS(
     ...
     * permission>
     */
    fun getPermissionsAssignedToRole(): HashMap<String, HashSet<Operation>> {
        ...
        policy.rules.forEach { rule ->
            ...
            permissionsByResource.getOrPut(resourceName) { hashSetOf() }.add(Operation.valueOf(permission))
        }
        return permissionsByResource
    }

    /**
     * Add the [operation] over the [resourceName]
    ...
    fun addPermissionToRole(
        ...
        val newPermission = XACMLRule(
            ruleId = "Permission:to:$operation:resource:$resourceName",
            target = XACMLTarget(
                anyOfs = mutableListOf(
                    ...
                    XACMLAnyOf(
                        allOfs = if (operation == Operation.READWRITE) {
                            mutableListOf(
                                XACMLAllOf(
                                    match = XACMLMatch(
                                        matchID = "urn:oasis:names:tc:xacml:1.0:function:string-equal",
                                        attribute = XACMLAttributeValue(
                                            dataType = "http://www.w3.org/2001/XMLSchema#string",
                                            attributeValue = Operation.READ.toString(),
                                        ),
                                        designator = XACMLAttributeDesignator(
                                ...
                                XACMLAllOf(
                                    match = XACMLMatch(
                                        ...
                                        attribute = XACMLAttributeValue(
                                            dataType = "http://www.w3.org/2001/XMLSchema#string",
                                            attributeValue = Operation.WRITE.toString(),
                                        ),
                                        designator = XACMLAttributeDesignator(
                                            category = "urn:oasis:names:tc:xacml:3.0:attribute-category:action",
                                            attributeId = "urn:oasis:names:tc:xacml:1.0:action:action-id",

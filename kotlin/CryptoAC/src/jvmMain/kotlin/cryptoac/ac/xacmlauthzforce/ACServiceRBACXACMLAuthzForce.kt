...
import cryptoac.ac.xacmlauthzforce.XACMLPolicySet.Companion.getPPSIDByRole
import cryptoac.ac.xacmlauthzforce.XACMLPolicySet.Companion.getREPSIDByRole
import cryptoac.ac.xacmlauthzforce.XACMLPolicySet.Companion.getRPSIDByRole
import cryptoac.ac.xacmlauthzforce.XACMLPolicySet.Companion.getRoleNameFromPolicySetID
import cryptoac.ac.xacmlauthzforce.XACMLPolicySet.Companion.rootDefaultPolicySetId
import cryptoac.ac.xacmlauthzforce.XACMLPolicySet.Companion.rootPolicySetIdCryptoAC
import cryptoac.core.CoreParameters
import cryptoac.code.CodeBoolean
import cryptoac.code.CodeServiceParameters
import cryptoac.code.CodeString
 ...
 * AC AuthzForce XACML PDP server configured with the given
 * [acServiceParameters] for RB-CAC
 */
class ACServiceRBACXACMLAuthzForce(
    ...
    override fun init() {
        logger.info { "No action required to initialize the AC RBAC XACML service" }
    }
    ...
    override fun addUser(
        ...
        return CodeServiceParameters(
            serviceParameters = ACServiceRBACXACMLAuthzForceParameters(
    ...
    override fun canDo(
        ...
        logger.info { "Keep roles having operation $operation over resource $resourceName" }
        ...
        run error@{
            roleAssignedToUser.forEach { currentRoleName ->
                logger.info {
                    ...
                    "operation $operation over the resource " +
                        ...
                        XACMLAttributes(
                            ...
                            attributes = mutableListOf(
                                XACMLAttribute(
                                    ...
                                    attributeValue = XACMLAttributeValue(
                                        dataType = "http://www.w3.org/2001/XMLSchema#string",
                                        attributeValue = operation.toString()
                                    )
                                )
                            )
                        ),
        ...
        return if (roleHavingPermissionOnResource.isEmpty()) {
            ...
            logger.info {
                "User $username has operation $operation over the resource " +
